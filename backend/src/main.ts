...
//function globalJwtGuard(app: INestApplication): void {
//  const reflector: Reflector = app.get(Reflector);
//  const jwt: JwtService = app.get(JwtService);
//  app.useGlobalGuards(new JwtGuard(reflector, jwt));
//}

function initOpenAPI(app: INestApplication, configService: ConfigService): void {
...
async function bootstrap() {
  ...
  app.enableCors({
    allowedHeaders: ['Location', 'content-type', 'access-control-allow-headers', 'Authorization'],
    maxAge: 600
  });
