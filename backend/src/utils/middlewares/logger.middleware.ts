...
export class LoggerMiddleware implements NestMiddleware {
    ...
    fileTest(request: Request, response: Response,): string {
        ...
        Object.keys(request.headers).forEach(headerKey => {
            content = content.concat(`\t${headerKey}: ${request.headers[headerKey]}\n`)
        });
        content = content.concat('}\n');
        content = content.concat('\nBody: {\n');
    ...
    use(request: Request, response: Response, next: NextFunction): void {
        ...
        response.on('finish', () => {
            ...
            if (statusCode >= 500) {
                this.logger.error(
                    `${method} ${originalUrl} ${statusCode} [${ip}] - ${responseTime - requestTime}ms`,
                    );
                const _this = this;
                writeFile(`log/${requestTime}`, this.fileTest(request, response), function (err) {
                    if (err) throw err;
